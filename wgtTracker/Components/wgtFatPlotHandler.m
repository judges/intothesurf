...
@implementation wgtFatPlotHandler
...
-(void)initAPlot:(CPScatterPlot*)inPlot
{
	...
	plot.dataLineStyle.lineColor = [CPColor orangeColor];
	...
	blueCirclePlotSymbol.fill = [CPFill fillWithColor:[CPColor redColor]];
...
-(NSNumber*)NumberForRecordIndex:(NSUInteger)index
{
	...
	int value = wgtEntry.Fat;
	
	value -=minValue;
	return [NSNumber numberWithInt:10+ value / factor];
}
